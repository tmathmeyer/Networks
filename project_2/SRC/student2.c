...
void B_input(struct pkt packet) {
	...
	{ //the packet is valid!!
		...
		return_packet.acknum=1;
		strncpy(return_packet.payload, packet.payload, MESSAGE_LENGTH);
		return_packet.checksum = packet.checksum;
		return_packet.seqnum = packet.seqnum;

		tolayer3(1, return_packet);
		tolayer5(1, m);
	}
	else
